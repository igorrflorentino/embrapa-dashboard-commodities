...
import './filtersSchema.js'; // registra window.ORIGEM_OPTIONS + window.labelProductRows

const CARVAO_SILV = { code: '3455', name: 'Carvão vegetal', origem: 'silvicultura', value: 114564 };
const CARVAO_EXTR = { code: '3433', name: 'Carvão vegetal', origem: 'extrativa', value: 13745 };
const ACAI = { code: '3403', name: 'Açaí (fruto)', origem: 'extrativa', value: 1 };
...
describe('labelProductRows — desambiguar só quando é preciso', () => {
  ...
  it('o rótulo curto vem do MESMO registro que o filtro usa', () => {
    // Âncora externa: se alguém renomear uma metade no ORIGEM_OPTIONS, o sufixo do gráfico
    // acompanha. Sem isso nasceria um segundo vocabulário da metade do PEVS.
    const opts = window.ORIGEM_OPTIONS.ibge_pevs.filter((o) => o.short);
    expect(opts.length).toBe(2);
    for (const o of opts) {
      const out = window.labelProductRows(
        [{ name: 'X', origem: o.value }, { name: 'X', origem: 'outra' }], 'ibge_pevs',
      );
