{#-
    The SILVICULTURE half of PEVS (SIDRA t291, planted forest) — the sibling of
    silver_ibge_pevs, which carries the EXTRACTION half (t289, native forest).

    Deliberately a mirror rather than a parameterized shared model: the two feed one
    Gold table (`gold_pevs_production`, discriminated by `origem`) but they read
    different Bronze tables with different product-code columns, and keeping each
    model's incremental contract local is what let PAM and PPM ship without touching
    the live PEVS path. Every note below applies to this model exactly as written for
    that one — including the measured non-incrementality of the `>=` boundary.

    See PLANS/silvicultura_source.md.
-#}
...
{#-
    ...
    4.4M live) and **69.9%** of bronze_pam.sidra_t5457_raw (39.1M vs 16.9M). This table was
    created 2026-08-29 and carries no dead weight YET; it accumulates the same way. Bronze is
    ...
    `dbt build --select silver_ibge_silvicultura+ --full-refresh`.
-#}
...
deduplicated as (
    ...
    select b.*
    from {{ source('bronze_ibge', 'sidra_silvicultura_raw') }} b
    inner join affected_years ay on b.ano = ay.ano
    where b.variavel_codigo in (
              '{{ var("silvicultura_variable_quantity") }}',
              '{{ var("silvicultura_variable_value") }}'
          )
    ...
    -- history and guarded downstream by assert_silvicultura_conserved_silver_to_gold, so this is defensive.
    qualify row_number() over (
        partition by b.ano, b.municipio_codigo, b.tipo_de_produto_da_silvicultura_codigo, b.variavel_codigo, lower(trim(b.unidade_de_medida))
...
with deduplicated as (
    ...
    qualify row_number() over (
        partition by ano, municipio_codigo, tipo_de_produto_da_silvicultura_codigo, variavel_codigo, lower(trim(unidade_de_medida))
...
select
    ...
    -- 291, a metade de silvicultura do PEVS.
    ...
    '{{ var("silvicultura_table_id") }}' as tabela,
    p.reference_year,
    p.city_code,
    p.city_name,
    p.state_acronym,
    ...
    case
        when p.variable_code = '{{ var("silvicultura_variable_value") }}' then
            p.raw_numeric_value * fx.brl_factor
        else
            p.raw_numeric_value
    end as numeric_value,

    case when p.variable_code = '{{ var("silvicultura_variable_value") }}' then true else false end as is_monetary_value,
    ...
    case when p.variable_code = '{{ var("silvicultura_variable_quantity") }}'
...
-- source='_reference' sentinel rows (which this 'pevs' filter excludes by design),
-- so nothing is overridden yet — see seeds/_seeds.yml. `source='pevs'` is deliberate
-- and not a copy-paste slip: the seed's source is the BANCO, and silviculture shares
-- `ibge_pevs` with the extraction half. The product codes differ (3455-3457 vs
-- 3433-3435), so one namespace cannot cross-apply an override to the wrong half.
