...
def test_the_tabela_axis_is_declared_in_both_filter_schemas() -> None:
    ...
    assert any(d.get("id") == "origem" for d in dims), "backend: sem a dimensão tabela"
    assert "id: 'tabela'" in SCHEMA_JS.read_text("utf-8"), "SPA: sem a dimensão tabela"
