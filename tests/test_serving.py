...
def test_products_readers_carry_the_pevs_half_only_when_asked(
    ...
    """``include_origem`` faz a metade do PEVS viajar junto com cada linha — e só quando
    ...
    ligada, _ = builder(table, include_origem=True, **comum)
    assert "any_value(origem) as origem" in ligada.lower()

    desligada, _ = builder(table, **comum)
    assert "origem" not in desligada.lower().split("from")[0], (
        "sem include_origem o select não pode citar origem — o COMEX não tem a coluna"
    )
